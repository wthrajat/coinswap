...
use bdk_chain::{
    ...
    Append, CombinedChangeSet, ConfirmationTimeHeightAnchor, IndexedTxGraph,
};
use bdk_wallet::{
...
impl Wallet {
    pub fn init(
        ...
        // Append the changeset generated during wallet initialization to the wallet's stage.
        wallet.stage.append(ChangeSet {
    ...
    pub fn load(data_dir: PathBuf, rpc_config: &RPCConfig) -> Result<Self, WalletError> {
        ...
        // Save wallet state: only updates meta_store.
        wallet.save()?;
        Ok(wallet)
    }
    ...
    pub fn get_new_address(&mut self, keychain: &Keychain) -> Result<Address, WalletError> {
        ...
        // Append the changeset generated to the wallet's stage.
    ...
    pub fn save(&mut self) -> Result<(), WalletError> {
        ...
        if let Some(changeset) = self.stage.take() {
